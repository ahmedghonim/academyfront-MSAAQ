...
const CertificateLibraryCard = ({ certificate }: CertificateCardProps) => {
  ...
  return (
    <Card
      className={`group h-full ${
        certificate.course
          ? "transform duration-150 ease-linear hover:-translate-y-2 hover:shadow-[12px_32px_32px_0px_rgba(0,0,0,0.12)]"
          : ""
      }`}
    >
      <Card.Body className="relative flex h-full flex-col p-4">
        ...
        <div className="flex flex-1 flex-col space-y-2">
          <div className="flex items-start justify-between gap-4">
            ...
            >
              {certificate.course ? (
                <ProgressBarLink href={`/courses/${certificate.course.slug}`}>
                  <span
                    aria-hidden="true"
        ...
        <Button
          onPress={() =>
            downloadFile(
              ...
              certificate.course ? `${certificate.course.title}-certificate` : `${certificate.serial}-certificate`
            )
          }
